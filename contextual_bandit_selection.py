...
class MetaBandits4Rec:
    ...
    --------
    user_id : string
        User id to label the bandit (each user works as a contextual bandit selection).
    d : int
        Length of the context, i.e. dimension of the context vector and mu parameter.
    context : 2D np.array
        Matrix of the contexts. Each row represent an arm (e.g. the algorithm) to be selected based on its features.

    '''

    def __init__(self, user_id, d, context):
    ...
    def UCB_get_expected_reward(self, alpha=1):
        '''
